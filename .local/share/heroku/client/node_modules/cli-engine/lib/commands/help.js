...
Help topics, type ${this.out.color.cmd(this.config.bin + ' help TOPIC')} for more details:\n`);
    ...
    topics = topics.map(t => [t.topic, this.out.color.gray(t.description)]);
  ...
  listNamespaceHelp(plugins) {
    ...
    for (var i = 0; i < plugins.length; i++) {
      ...
      if (plugin.topics) {
        this.out.log(renderList(plugin.topics.filter(t => !t.hidden).map(t => [t.topic, this.out.color.gray(t.description)])));
      }
    }
  }
